(function () {
    ...
    //window.sessionStorage.notNowSince = window.sessionStorage.notNowSince || -1;
    var SubscribeNewsletter = function(scrollPos){
        if(window.sessionStorage.inviteToSubscription == "false") return;
        let windowHeight = $(window).height();
        if(scrollPos >= windowHeight) {
            $('aside.newsletter-subscribtion').addClass('opened');
        }else{
            $('aside.newsletter-subscribtion').removeClass('opened');
        }
    }

    // var resetNewsLetterTemp = function (){
    //     let notNowCounter = setInterval(function(){
    //         window.sessionStorage.notNowSince++;
    //         if(window.sessionStorage.notNowSince >= 20){
    //             clearInterval(notNowCounter);
    //             window.sessionStorage.inviteToSubscription = true;
    //         }
    //     },60000);
    // }

    // if (window.sessionStorage.notNowSince != -1) resetNewsLetterTemp();
    ...
    var closeNewsletterSubscription = function(e,temp){
        ...
        // if(temp){
        //     window.sessionStorage.notNowSince = -1;
        //     resetNewsLetterTemp();
        // }
    }
    ...
    var newsLetterSubmit = function(e){
        ...
        if($(this).closest('footer').length){
            //$('aside.newsletter-subscribtion').removeClass('opened');
            $('.newsletter-subscribtion input').val($(this).find('input').val());
            $('aside.newsletter-subscribtion form').submit();
            return;
        }
        $('.newsletter-subscribtion button i').fadeOut(function(){
            $('.newsletter-subscribtion').addClass('submitted').find('form button span').delay(300).fadeIn(function(){
                $('.newsletter-subscribtion input').val('');
                $(this).delay(500).fadeOut('fast',function(){
                    $('.newsletter-subscribtion').removeClass('submitted').find('button i').delay(400).fadeIn();
                });
            });
        });
    }
        ...
        if ($('#main-carousel').length) {
            ...
            if (scrollPos >= Math.floor(breakPos)) {
                if (newPaddingValue <= carouselBodyHeight) {
                    ...
                    $('.carousel-data').css('margin-top', (newTopValue * -0.5));
                }
            } else {
            ...
            var carouselBodyHeight = $('.details-carousel').height(),
                carouselOffsetTop = $('.details-carousel').offset().top * 0.75,
        ...
        var checkPos = function () {
            $('.show-onscroll').each(function () {
                ...
                var docScroll = $(document).scrollTop(),
                    ...
                    startFadeIn = $(window).height() * 0.75;
    ...
    var initTextScroll = function () {
        var distributeOpacity = function () {
            $('.text-scroll .text-scroll-unit-content>*').each(function () {
                var docScroll = $(document).scrollTop(),
                    posTop = $(this).offset().top - docScroll,
                    startFadeIn = $(window).height() * 0.65,
                    startFadeOut = $(window).height() * 0.3,
                    newOpacity;
                if (posTop < startFadeIn) {
    ...
    var InitPopupGallery = function () {
        if (lastScreenSize != newScreenSize) {
            if (newScreenSize == 1) {
                imgSmallSliderContainer.find('.img-holder').off().on('click', function () {
                    DisplayClickedPopupGalleryItem($(this).find('img,iframe').attr('popup-slide-data-index'));
                });
                ...
                imgSmallSliderContainer.find('.img-holder').off();
    ...
    var PopulateGalleryPopup = function () {
        ...
        for (var i = 0; i < PopupGallerySlides.length; i++) {
            if (PopupGallerySlides[i].type && PopupGallerySlides[i].type == 'video') {
                var src = PopupGallerySlides[i].src.split('?');
                imgSmallSliderContainerHtml += '<div class="img-holder"><iframe width="100%" height="100%" src="' + PopupGallerySlides[i].src + '" frameborder="0" allow="encrypted-media" popup-slide-data-index="' + i + '" class="slider-slide-image" allowfullscreen></iframe></div>';
            } else {
    ...
    var DisplayClickedPopupGalleryItem = function (imageIndex) {
        ...
        sliderMainImage.hide().delay(100).queue(function () {
            sliderMainImage.remove();
            if (PopupGallerySlides[PopupImgToDisplayIndex].type && PopupGallerySlides[PopupImgToDisplayIndex].type == 'video') {
    ...
    var MoveGalleryPopupSlider = function (prevImageIndex, clickedImageIndex) {
        if (PopupGallerySlides.length > 7) {
            ...
            var imgHolderWidth = $('.img-small-slider-container .img-holder').innerWidth(),
                differenceToEnd = PopupGallerySlides.length - 1 - clickedImageIndex,
                differenceToStart = clickedImageIndex - 0;
    ...
    var PopulatePopup = function (type, title) {
        ...
        $('#popup-base').find('.' + type + '-container').css('display', 'inline-block').hide().fadeIn(0);
    ...
    var ChangeSelectedCat = function (obj) {
        let parent = obj.closest('.search-input-filter-wrp');
        parent.find('.search-filter-menu li').removeClass('active');
        let selectedCat = obj.closest('li').addClass('active').text().trim();
        parent.find('.search-filter-display').text(selectedCat);
    ...
    var InitCatDropdown = function () {
        if ($('.search-input-filter-wrp').length) {
            $('.search-input-filter-wrp').find('.search-filter-display').off().on('click', function () {
                $(this).closest('.search-input-filter-wrp').toggleClass('active');
            });
    ...
    //-------------------------------------------------------------------------------------------------
    //Init event gallery details

    var InitEventDetailsGallerySlider = function () {
        $('.gallery-events-wrp').find('.gallery-events-item').each(function () {
            var element = {};
            if ($(this).find('img').length) {
                element.src = $(this).find('img').attr('src');
                element.title = $(this).find('img').attr('title');
                element.type = 'img';
            } else {
                element.src = $(this).find('iframe').attr('src');
                element.title = '';
                element.type = 'video';
            }
            DetailsGallerySlides.push(element);
        });
        var counter = 0;
        //$('.main-image').attr('src', DetailsGallerySlides[counter].src).attr('alt', DetailsGallerySlides[counter].alt).attr('data-index', counter);
        $('.gallery-events-item').each(function () {
        ...
        $('.gallery-events-item').off().on('click', function () {
            var clickedImgIndex;
            IsDetailsGalleryPaused = true;
            clickedImgIndex = $(this).attr('data-index');
            DrawPopupGallery(DetailsGallerySlides, clickedImgIndex);
            InitPopupGallery();
        });
        AutoPlayDetailsGallery();
    ...
    $(document).ready(function () {
        ...
        $('.newsletter-subscribtion .subscription-invition').on('click',function(){$('.newsletter-subscribtion').addClass('opened');})
        $('.newsletter-subscribtion form').on('submit',newsLetterSubmit);
        $('footer .newsletter-subscribtion form input').on('focus',closeNewsletterSubscription);

        //Scroll to id
        if (window.location.href.indexOf('scrollto=') > -1) {
            var strMainFraction = window.location.href.split('scrollto=');
            var strSubFraction = strMainFraction[1];
            if (strMainFraction[1].indexOf('&') > -1) {
                strSubFraction = strMainFraction[1].split('&');
                strSubFraction = strSubFraction[0];
            }
            if ($('#' + strSubFraction).length) {
                var heightToSub = 0;
                var additionalHeight = 10;
                var bottomHeaderHeight = 0;
                var attachedMenuHeight = $('.attached-menu').height();
                if ($(window).width() > 1024) {
        ...
        $('.add-comment-btn').on('click', function () {
            PopulatePopup('comment', 'Comment');
            $('html,body').addClass('popup-in-motion');
        });

        $('.upload-photo-btn').on('click', function () {
            $(this).closest('.comment-submit-wrp').find('input[type=file]').click();
        });

        $("#comment-images").change(function () {
        ...
        $('#popup-close-btn').on('click', function () {
            $('#popup-base').removeClass('popup-active search-popup-active comment-popup-active').find('.close-btn').removeClass('init');
        ...
        if ($('.pagination-container').length) paginate();
                ...
                $('#main-carousel .carousel-container').on('mousemove touchmove', function (e) {
                    if (carouselDrag && !scrollingOnCarousel) {
                        ...
                        if (carouselPosDiff > 0) {
                            ...
                            carouselItemTarget.next().find('.carousel-data').css('left', fullScroll - (carouselPosDiff * 0.5) + 'px');
                        } else {
                            ...
                            carouselItemTarget.prev().find('.carousel-data').css('left', -650 - (carouselPosDiff * 0.5) + 'px');
                        }
